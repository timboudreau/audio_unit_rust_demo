...
public class SimplePlayEngine {
    ...
    func findComponent(_ type: String, _ subType: String, _ manufacturer: String) -> AVAudioUnitComponent {
        for i in 0...10 {
            reset();
            print("Find component attempt \(i + 1)")
            let c = AVAudioUnit.findComponent(type: type, subType: subType, manufacturer: manufacturer);
            if c != nil {
                print("Success!");
                return c!
            } else {
                print("Find component attempt \(i + 1) failed")
                sleep(1)
            }
        }
    ...
    func initComponent(type: String, subType: String, manufacturer: String, completion: @escaping (Result<Bool, Error>, ViewController?) -> Void) {
        ...
        // Otherwise this can race with registration
        sleep(1)

        let component = findComponent(type, subType, manufacturer)

        // Instantiate the audio unit.  Here we pass loadInProcess, and in this version of the project
        // we expect it to SUCCEED, since this project *is* set up for running in-process.
        AVAudioUnit.instantiate(with: component.audioComponentDescription,
                                options: AudioComponentInstantiationOptions.loadInProcess) { avAudioUnit, error in
