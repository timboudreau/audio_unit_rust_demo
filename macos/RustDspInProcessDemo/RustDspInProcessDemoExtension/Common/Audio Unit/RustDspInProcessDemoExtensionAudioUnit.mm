#import "RustDspInProcessDemoExtensionAudioUnit.h"

#import <AVFoundation/AVFoundation.h>
#import <CoreAudioKit/AUViewController.h>

#import "RustDspInProcessDemoExtensionBufferedAudioBus.hpp"
#import "RustDspInProcessDemoExtensionAUProcessHelper.hpp"
#import "RustDspInProcessDemoExtensionDSPKernel.hpp"

#import <mock_dsp_lib.h>

@interface RustDspInProcessDemoExtensionAudioUnit ()
...
@implementation RustDspInProcessDemoExtensionAudioUnit {
    // C++ members need to be ivars; they would be copied on access if they were properties.
    RustDspInProcessDemoExtensionDSPKernel _kernel;
    BufferedInputBus _inputBus;
    std::unique_ptr<AUProcessHelper> _processHelper;

    // Addresses a race condition in the Apple template - the plugin can be deinitialized
    // while it is inside the render block
    uint8_t _deinitSpinlock;
}
...
- (NSArray<NSNumber *> *)channelCapabilities {
    // In this demo we are only supporting stereo
    NSNumber *two = [NSNumber numberWithInt:2];
    NSArray *array = [NSArray arrayWithObjects:two, two, nil];
    return array;
}
...
- (void)setupParameterCallbacks {
    ...
    __block RustDspInProcessDemoExtensionDSPKernel *kernel = &_kernel;
...
- (void)deallocateRenderResources {
    ...
    // There is a race condition in the original Apple AudioUnit template which
    // causes deinitialization while the render loop is running.  While this mainly
    // happens if the current preset is changed during playback, including it for
    // correctness.
    while (1 == read_u8_atomic(&_deinitSpinlock)) {
        sleep(5);
    }
...
- (AUInternalRenderBlock)internalRenderBlock {
    ...
    __block uint8_t *spinlock = &_deinitSpinlock;
